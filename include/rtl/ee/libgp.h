...
static __inline__ unsigned int sceGpIndexRgbaLineF(unsigned int n)         { return n*3;       }
static __inline__ unsigned int sceGpIndexXyzfLineF(unsigned int n)         { return n*3/2 + 1; }
static __inline__ unsigned int sceGpIndexUvLineFTU(unsigned int n)         { return n*5/2 + 1; }
static __inline__ unsigned int sceGpIndexRgbaLineFTU(unsigned int n)       { return n*5;       }
static __inline__ unsigned int sceGpIndexXyzfLineFTU(unsigned int n)       { return n*5/2 + 2; }
...
static __inline__ unsigned int sceGpIndexRgbaTriF(unsigned int n)          { return n*4;       }
static __inline__ unsigned int sceGpIndexXyzfTriF(unsigned int n)          { return n*4/3 + 1; }
static __inline__ unsigned int sceGpIndexUvTriFTU(unsigned int n)          { return n*7/3 + 1; }
static __inline__ unsigned int sceGpIndexRgbaTriFTU(unsigned int n)        { return n*7;       }
static __inline__ unsigned int sceGpIndexXyzfTriFTU(unsigned int n)        { return n*7/3 + 2; }
...
static __inline__ unsigned int sceGpIndexRgbaSpriteF(unsigned int n)       { return n*3;       }
static __inline__ unsigned int sceGpIndexXyzfSpriteF(unsigned int n)       { return n*3/2 + 1; }
static __inline__ unsigned int sceGpIndexUvSpriteFTU(unsigned int n)       { return n*5/2 + 1; }
static __inline__ unsigned int sceGpIndexRgbaSpriteFTU(unsigned int n)     { return n*5;       }
static __inline__ unsigned int sceGpIndexXyzfSpriteFTU(unsigned int n)     { return n*5/2 + 2; }
...
extern const int _sce_Index_XyzFTS[7][3];
...
static __inline__ unsigned int sceGpIndexUv(u_int type, unsigned int n)
{
    const int *p;
    if (type & SCE_GS_PRIM_IIP){ /* gouraud */
        return n*3;
    } else {                     /* flat */
        if( type & SCE_GP_MONOCHROME ){
            return n*2;
        } else {
            p = _sce_Index_UvFTU[type & GS_PRIM_PRIM_M];
            return n * (*p) / (*(p+1)) + *(p+2);
        }
    }
}
...
static __inline__ unsigned int sceGpIndexRgba(u_int type, unsigned int n)
{
    const int *p;
    if (type & SCE_GS_PRIM_IIP){ /* gouraud */
        if ( type & SCE_GS_PRIM_TME )   return n*3 + 1;
        else                            return n*2;
    } else {                     /* flat */
        if ( type & SCE_GS_PRIM_TME ){  /* w/ texture */
            if ( type & SCE_GS_PRIM_FST ){  /* uv */
                p = _sce_Index_RgbaFTU[type & GS_PRIM_PRIM_M];
                return n * (*p) / (*(p+1)) + *(p+2);
            } else {                        /* stq */
                p = _sce_Index_RgbaFTS[type & GS_PRIM_PRIM_M];
                return n * (*p) / (*(p+1)) + *(p+2);
            }
        } else {                        /* w/o texture */
            p = &(_sce_Index_RgbaF[type & GS_PRIM_PRIM_M][0]);
            return n * (*p) / (*(p+1)) + *(p+2);
        }
    }
}

static __inline__ unsigned int sceGpIndexXyzf(u_int type, unsigned int n)
{
    const int *p;
    if (type & SCE_GS_PRIM_IIP){ /* gouraud */
        if ( type & SCE_GS_PRIM_TME )   return n*3 + 2;
        else                            return n*2 + 1;
    } else {                     /* flat */
        if( type & SCE_GP_MONOCHROME ){ /* monochrome */
            if ( type & SCE_GS_PRIM_TME )  return n*2 + 1;  /* w/ texture  */
            else                           return n;    /* w/o texture */
        } else {                        /* not monochrome */
            if ( type & SCE_GS_PRIM_TME ){  /* w/ texture */
                if ( type & SCE_GS_PRIM_FST ){  /* uv */
                    p = _sce_Index_XyzFTU[type & GS_PRIM_PRIM_M];
                    return n * (*p) / (*(p+1)) + *(p+2);
                } else {                        /* stq */
                    p = _sce_Index_XyzFTS[type & GS_PRIM_PRIM_M];
                    return n * (*p) / (*(p+1)) + *(p+2);
                }
            } else {                        /* w/o texture */
                p = _sce_Index_XyzF[type & GS_PRIM_PRIM_M];
                return n * (*p) / (*(p+1)) + *(p+2);
            }
        }
    }
...
#define sceGpSetXy(p, k, x, y) ((p)->reg[k].xyzf.X=(x),(p)->reg[k].xyzf.Y=(y))
#define sceGpSetXyz(p, k, x, y, z) ((p)->reg[k].xyzf.X=(x),(p)->reg[k].xyzf.Y=(y),(p)->reg[k].xyzf.Z=(z))
#define sceGpSetXyzf(p, k, x, y, z, f) ((p)->reg[k].xyzf.X=(x),(p)->reg[k].xyzf.Y=(y),(p)->reg[k].xyzf.Z=(z),(p)->reg[k].xyzf.F=(f))
